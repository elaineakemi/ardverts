...
import { CAMERA_HEIGHT, CAMERA_WIDTH, CANVAS_HEIGHT, CANVAS_WIDTH } from '../../utils/consts'
  ...
  useEffect(() => {
    const loadResources = async () => {
      try {
        ...
        const camera = new threejs.PerspectiveCamera(75, width / height, 0.1, 1000)
        camera.position.z = 5
        const renderer = new threejs.WebGLRenderer({ canvas: canvasRef.current, alpha: true })
  ...
  useEffect(() => {
    const detectAndPositionGlasses = async () => {
      ...
      if (faceEstimates.length > 0) {
        ...
        const eyeLine = new threejs.Vector2(rightEye[0] - leftEye[0], rightEye[1] - leftEye[1])
    ...
    <>
      ...
      <div style={{ position: 'relative', margin: '0 auto' }}>
        ...
        <Webcam
          ...
          autoPlay
          playsInline
          style={{ width: CANVAS_WIDTH, height: CAMERA_HEIGHT }}
          mirrored={true}
        />
        <canvas
          ref={canvasRef}
          style={{ maxWidth: CANVAS_WIDTH, height: CANVAS_HEIGHT, position: 'absolute', top: 0, left: 0 }}
        />
      </div>
    </>
  )
